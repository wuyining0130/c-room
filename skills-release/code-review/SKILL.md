...
- **代码变更**：以下方式之一：
  - Git 状态与 diff（默认覆盖 staged、unstaged、untracked，并按仓库确定基线分支）
  - 指定文件列表
...
- 如果用户明确指定文件或提交范围 → 以其为主，同时检查范围内仓库是否还有相关未跟踪/未提交变更并提示
- 否则对每个目标仓库同时审查 `merge-base(远端默认分支, HEAD)..HEAD` 的已提交变更，以及 staged、unstaged、untracked 文件
- 优先读取远端默认分支或仓库配置；无法确定基线分支时再询问，不硬编码 main/master
- `code-gen-report.md` 仅用于把实际变更与计划交叉检查，不能替代真实 Git 状态
- 审查前向用户确认范围；只读检查不因存在脏工作区而停止
...
- 校验 tech-design 记录的 PRD 版本与当前 PRD 一致；不一致列为审查基准风险
- 验证 coding-knowledge 中引用的路径、签名和关键调用链仍与当前源码一致
- 读取 code-gen-report 的实际测试/构建结果；缺失时补跑安全且相关的验证命令
- 验证前后分别记录 Git 状态；命令产生计划外文件时报告并与用户原改动区分，不擅自清理
...
- 检查变更是否包含与新增/修改行为相匹配的测试；不能仅以“已有测试目录”判定覆盖
- 运行受影响模块的最小相关测试、类型检查、lint 和构建，记录命令与结果
- 区分代码失败、环境失败、既有失败；没有执行证据时不得把“测试通过”作为结论
- 测试覆盖不足按行为风险分级：关键业务规则或安全路径无测试可标阻塞，一般分支缺测标建议
...
prd_path: "{路径}"
prd_version: "{版本}"
prototype_path: "{路径或 null}"
review_baseline: "{默认分支、merge-base、HEAD、工作区状态摘要}"
knowledge_baseline: "{各仓库扫描基线摘要}"
---
...
| 仓库 | 检查项 | 命令 | 结果 | 说明 |
|------|--------|------|------|------|
| {仓库} | 单元测试/类型检查/lint/构建 | `{实际命令}` | 通过/失败/未执行 | {关键结果或阻塞原因} |
