...
- 原型文件路径（canonical 路径为 `requirements/{模块名}/prototype/{模块名}.html`）
...
提取以下信息：
- 功能清单（功能编号、名称、优先级）
- 需求详情（每个功能的交互、业务规则与边界、错误处理、验收标准）
- 各 F-XX 内的数据定义或字段配置（新增/修改的业务实体和字段）
- 业务流程图（操作流程、状态流转）
- 权限矩阵（影响鉴权逻辑的部分）
- **待确认项列表**（PRD 末尾的 Q-XX 项）——技术方案中如涉及同一问题，应保持结论方向一致或显式说明分歧原因，并引用 PRD 待确认项编号（如"关联 PRD Q-01"）

**质量门禁**：提取 PRD 路径、版本和 mode。standard 模式必须读取 `review/review-summary.md`，确认报告对应当前 PRD 版本且阻塞为 0；mini 模式执行轻量自检。报告缺失、过期或有阻塞时停止生成。

#### 1.2 读取原型

用户未指定原型时，先按 PRD 的 module 查找 canonical `{模块名}.html`：
- 正好存在 → 读取
- 不存在 → 若需求包含前端页面，停止并提示先生成原型；纯后端需求可明确记录“不适用”后继续
- canonical 不存在但目录有其他 HTML，或存在多个候选 → 不猜测，列出候选请用户选择

读取 HTML 顶部来源注释，确认其 PRD 路径和版本与当前 PRD 一致；不一致时视为过期原型，停止生成或先更新原型。
...
1. 读取相关仓库的扫描基线（如 `last_scanned_commit`）；基线缺失或落后于当前 HEAD 时，将知识库定位标为“待源码验证”。
2. 对最终写入方案的每个现有文件、类、方法和接口路径，从当前源码验证存在性和签名；行号以当前源码为准。
3. 检查 PRD、原型、coding-knowledge 与源码是否冲突，按 conventions Section 7.5 处理。
4. 会改变业务行为、接口契约或数据口径的冲突必须先停止生成并请用户确认；不得在方案中静默选择一个版本。
...
**出入参完整性是接口设计的硬性要求**——从 PRD 第四章各 F-XX 的交互、业务规则、数据定义和原型的表单/表格字段提取完整出入参。对修改现有接口的场景，标注新增和已有字段。
...
1. **新增表**：输出完整的 CREATE TABLE DDL
   ...
   - 不直接把 PRD 数据定义当成 DDL——那是业务视角，字段名可能是中文
...
frontmatter 至少记录：`prd_path`、`prd_version`、`prd_review_path`、`prototype_path`、`prototype_prd_version`、相关仓库的 `knowledge_baseline`。不适用项显式标注 `null`，不能省略到无法追溯。

技术方案自身使用 `version: tech-v1` 和 `status: draft`。每次修改递增 `tech-vN`；用户明确确认方案后才将 status 更新为 `approved`。code-gen 只实施 approved 版本。
...
- **业务补充**：`coding-knowledge/business/` 提供业务上下文；PRD 各 F-XX 的数据定义和流程图也是重要输入
...
**把 PRD 的数据定义直接当 DDL**：PRD 中的数据定义是业务视角，DDL 是实现视角。要参考当前源码和 database-schema.md 做翻译。

**遗漏间接影响**：一个功能的改动可能连锁影响其他服务。必须从 call-chains.md 沿着完整调用链追踪。特别注意中间层服务（BFF、编排层、网关）——它们最容易被遗漏。

**方案选型过度**：不是每个功能都需要方案对比。CRUD 功能不需要方案选型。

**正文中残留模糊措辞**：生成文档后自检，搜索“建议确认”“待确认”“后续确认”。出现在正文中（非第 9 节）的必须给出技术结论或移入待确认项。
