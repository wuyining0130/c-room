...
description: >-
  ...
  结合项目知识库自动识别需求描述中的模糊点，通过 8-15 轮带选项的澄清问题让用户确认细节，
  ...
  生成或修改 PRD 时，必须区分 1.1 背景与目标（现状痛点、目标与收益）和
  1.2 需求范围（本期能力边界、包含/不包含），避免两节互相复述或复制功能清单。
...
- 用户的需求描述（自然语言，可以很粗略）
- 项目知识库（`coding-knowledge/business/`，如有）
...
2. 如果 **存在** → 进入修改模式（跳过 Phase 1-4，直接执行修改模式的 M-Step 1-4）
   ...
   - 然后按 M-Step 1-4 执行
3. 如果 **不存在** 且用户意图是小需求 → 进入小需求模式（执行 Mini Flow）
4. 如果 **不存在** 且用户意图是完整 PRD → 进入首次生成模式（执行 Phase 1-4）

**如何判断小需求模式**：用户说"小需求"、"简单需求"、"写个简单的"、"不用走完整流程"、"只要背景和需求点"等，或者需求明显只涉及单一功能点、改动极小（如加一个字段、改一个规则），直接走小需求模式。如果拿不准，问一句"这个走简单模式还是完整 PRD？"。
...
1. **读取 PRD 参考知识库**：如果存在 `coding-knowledge/business/`，读取以下文件建立对现有系统的理解：
   - `system-overview.md` — 了解现有功能清单、运营后台交互模式、角色与权限
   - `business-flows.md` — 了解现有业务流程，新功能要嵌入现有体系
   - `glossary.md` — 了解业务术语（读"业务定义"部分）
...
5. **边界条件**（必问）：明确"本次不做什么"——功能范围的排除项、一期不包含的内容。避免生成的 PRD 范围失控。
6. **数据生命周期**（必问，涉及新增数据实体时）：新数据创建后如何变化？是否有状态流转？能否编辑/删除/归档？这类问题不问清楚，需求详情中的数据定义容易缺失。
7. **配置模型验证**（按需，涉及修改现有配置页面时必问）：当需求涉及在现有页面上新增字段或修改配置项时，应追问确认：
   - 新字段添加在哪个配置页面/表单上？
...
8. **数据关系**（按需）：涉及哪些数据对象？和现有数据的关系？
9. **与现有功能的关系**（按需，知识库中有相似功能时问）
10. **异常处理**（按需）：关键异常场景怎么处理？
11. **优先级**（按需）：如果功能较多，哪些是第一期必须做的？
12. **其他约束**（按需）：性能要求、兼容性要求等
...
"以上是目前的确认结果。还有什么需要补充或调整的吗？没有的话我开始生成。"

如果用户补充了新信息或提出调整：
- 进入追加澄清轮次（最多 5 轮追问）
- 追加轮次完成后更新总结，再次询问
- 最多迭代 2 次（即初始总结 + 至多 2 轮补充），防止无限循环

如果用户确认无补充（"没了"、"可以了"、"开始生成"）→ 进入 Phase 3 生成。
...
PRD 文档使用标准七章结构，详见 `references/prd-template.md`。如修改模式需要追加变更记录，作为文末非编号区保留。
...
- 1.1 背景与目标聚焦"为什么做"和"做成后的业务结果"，1.2 需求范围聚焦"本期做到哪些能力边界/不做哪些边界"，两节不得互相复述，也不得复制功能清单
- 每个 F-XX 必须有可测试的验收标准
- 每个 F-XX 内执行信息归属：功能描述、交互/规则、错误处理、验收标准各司其职；边界条件和错误处理无独立内容时省略
- 去重台账和临时编号仅用于生成时自检，最终 PRD 使用自然语言，不输出 I-XX、B-XX、E-XX、AC-XX 等内部编号，也不让读者跨小节解码引用
- 成稿后逐个 F-XX 检查语义重复：同一事实再次出现时，只有承担新的阅读目的（如提供测试场景和可观察结果）才保留，并用自然、完整但简洁的句子表达
- Mermaid 节点文字用双引号包裹
- 流程图覆盖异常分支，跨系统用泳道图
...
适用于改动极小、用户自己心里已经清楚要做什么的场景。跳过完整澄清流程，快速输出一份轻量需求文档。

### 触发条件

用户明确表示想走简单模式，或需求本身极其简单（单一功能点、加字段、改规则等）。

### 流程

1. **确认模块名**：问用户需求名称（用于目录 `requirements/{模块名}/`），如果用户已经说了就直接用
2. **快速澄清**（1-3 轮，仅在关键信息缺失时追问）：只问影响生成质量的核心问题，比如"谁用"、"在哪个页面"。用户描述已经足够清晰的就不问
3. **生成文档**：在 `requirements/{模块名}/` 下生成 `prd-draft.md`，仅包含两个章节：
...
> 版本：v1.0 | 日期：{YYYY-MM-DD} | 模式：小需求

## 1. 需求背景

{为什么要做这个事，解决什么问题，1-3 段}

## 2. 需求详情

{具体要做什么，按条目列出每个需求点，每条需求点需要足够具体可执行}
...
- 不生成 `prd-context.md`，不走 Phase 3.5 守护规则写入
- frontmatter 和 F-XX 编号必须保留，使 prd-review、proto-gen、tech-design 能识别版本和功能；下游看到 `mode: mini` 时按轻量结构解析，不要求补完整七章
- 如果后续用户要扩充为完整 PRD，走修改模式并将 `mode` 调整为 `standard`
...
**问题太多太细：** 15 轮是上限。如果发现模糊点超过 15 个，优先问影响范围最大的（范围、角色、核心流程），其余放到"待确认项"中。
