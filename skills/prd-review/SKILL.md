...
用户提供 PRD 文件路径（Markdown 格式）。PRD 通常包含需求背景、功能清单、业务流程图、需求详情、权限管理等章节，部分章节（如数据模型、待确认项、默认假设）可能因需求类型或定稿清理而不存在，按实际内容检查即可。
...
在 PRD 文件所在的业务模块目录下输出单个检查报告文件：

```
requirements/{模块名}/
├── prd-draft.md                    ← prd-draft 产出的草稿
└── review-report.md                ← 本 skill 产出的检查报告（单文件）
```
...
2. 各级问题数量
3. 是否可交付开发
4. 报告文件位置（`requirements/{模块名}/review-report.md`）
